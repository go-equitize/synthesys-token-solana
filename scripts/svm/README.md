...
```bash
yarn ts-node scripts/svm/router/get-ccip-fee.ts [options]
```
...
```bash
yarn ts-node scripts/svm/router/1_token-transfer.ts [options]
yarn ts-node scripts/svm/router/2_arbitrary-messaging.ts [options]
yarn ts-node scripts/svm/router/3_data-and-tokens.ts [options]
```
...
The scripts use configuration from `scripts/config/index.ts` which provides network endpoints and contract addresses.
